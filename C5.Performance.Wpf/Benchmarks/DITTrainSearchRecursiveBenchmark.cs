...
{
    public class DITTrainSearchRecursiveBenchmark : Benchmarkable
    {
        ...
        private int trainSearch(int trainId)
        {
            ...
            if (trainId < CollectionSize)
                return _intervalTrains.FindOverlapsRecursive(_trains[trainId]).Count() > 0 ? 1 : 0;
            // If the is is out of range search for a train we know is not in the collection.
            return _intervalTrains.FindOverlapsRecursive(_trainsNotInCollection[(trainId - CollectionSize)]).Count() > 0 ? 1 : 0;
        }
        ...
        {
            return "DIT Train Recursive Search";
        }
    }
}